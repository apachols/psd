...
from psd import RcodeHooks, date_scheduler
from psd.register import table_matcher
from psd.register.models import *
from psd.register.matchmaker import updateMatchRecords
...
def clearMatchQuestion( question_name ):
    ques = MatchQuestion.objects.filter( question=question_name )
    if len(ques) > 0:
        print( "Going to remove old Match Question '%s'" % (question_name, ) )
        for q in ques:
            q.delete()
    else:
        print( "Initializing new question '%s'" % (question_name,) )
...
def checkOrganization():
    ...
    for asite in sites:
        if asite.id != settings.SITE_ID:
            print( "WARNING: SITE_ID in settings.py should be set to %d to correspond with this site." % (asite, asite.id), )
        else:
...
def setupMatchQuestions(do_quest = None):
    ...
    if do_quest is None or "kinkinesses" in do_quest:
        clearMatchQuestion( "Kinkinesses" )
        kink = MatchQuestion( question="Kinkinesses" )
        kink.question_code = "kinkinesses"
        kink.save()
        kink.matchchoice_set.create( choice="kinky only", choice_code="K" )
        kink.matchchoice_set.create( choice="non-kinky only", choice_code="NK" )
        kink.matchchoice_set.create( choice="either is fine", choice_code="EI" )
        kink.save()
        
     
    if do_quest is None or "funk" in do_quest:
        clearMatchQuestion( "the Funkiness Question" )
    ...
    if do_quest is None or "hot" in do_quest:
        clearMatchQuestion( "the Hotness Question" )
    ...
    if do_quest is None or "identity" in do_quest:
        clearMatchQuestion( "the Identity Question" )
...
def makeTestDB( event_name, add_to_event=True ):
    """
    ...
    if len(events) > 0:
        print( "Already have a testing1 event.  Going to leave it." )
        evt = events[0]
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2:
        ...
        if action=="help" or action=="-h" or action=="--help":
            ...
            makeTestDB("testing1", False )
            makeTestDB( "kink1", True )
            checkOrganization()
